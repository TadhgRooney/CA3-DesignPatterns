...
public class Menu extends JFrame{
		  ...
		  if they are a new customer, or will ask them to log in if they are an existing customer or admin.*/
			...
			continueButton.addActionListener(new ActionListener(  ) {
				public void actionPerformed(ActionEvent ae) {
                    ...
                    switch (user) {
                        case "New Customer" -> createCustomerScreen();
                        case "Administrator" -> handleAdminLogin();
                        case "Customer" -> handleCustomerLogin();
                    }
				}
	...
	private String askForPassword() {
		while (true) {
			String passwordInput = JOptionPane.showInputDialog(f, "Enter 7 character Password:");

			if (passwordInput != null && passwordInput.length() == 7) {
	...
	//	content.add(deleteAccountPanel);
		...
		bankChargesButton.addActionListener(e -> applyBankCharges());
		interestButton.addActionListener(e -> applyInterest());
		editCustomerButton.addActionListener(e -> editCustomer());
		navigateButton.addActionListener(e -> navigateCustomers());
		summaryButton.addActionListener(e -> displayTransactionSummary());
		accountButton.addActionListener(e -> addAccountToCustomer());
	...
	private void showAccountSelection(Customer e){
		...
		continueButton.addActionListener(new ActionListener(  ) {
			public void actionPerformed(ActionEvent ae) {
				showCustomerSelection(e);
			}
		});
	}

	private void showCustomerSelection(Customer e){
	...
	private void withdrawMoney(Customer e) {
		boolean loop = true;
		boolean on = true;
		double withdraw = 0;
	...
	private void displayStatement(Customer e){
	...
	private void lodgeMoney(Customer e){
		boolean loop = true;
		boolean on = true;
		double balance = 0;
	...
	//Helper method to replace for loop looking for customer over and over again
